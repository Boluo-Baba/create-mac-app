...
def plot(a, name):
    ...
    plt.savefig('figures/' + name + '/figure1.png')
    ...
    plt.savefig('figures/' + name + '/figure2.png')
    ...
    plt.savefig('figures/' + name + '/figure3.png')
    ...
    plt.savefig('figures/' + name + '/figure4.png')
...
if __name__ == '__main__':
    ...
    for i in files:
        try:
            ...
            results_df[i] = [round(a.eoz_percent, 6), round(a.incircle_diatance, 6), round(a.min_distance, 6)] + \
                            [r1[3], r1[2], r1[1], r1[0]] + \
                            [r2[3], r2[2], r2[1], r2[0]] + \
                            ['(' + a.incircle_position[0] + ', ' + a.incircle_position[1] + ')']
            if not os.path.exists('figures/' + i[:-4]):
                os.makedirs('figures/' + i[:-4])
            plot(a, i[:-4])
        except:
            print('Cal ', i, ' failed!!!!!')

    results_df = results_df.T
    results_df.columns = ['index P', 'index D/mm', 'Dmin'] + \
                         ['Default mean1', 'Default angle1', 'Default mean2', 'Default angle2'] + \
                         ['Ablation mean1', 'Ablation angle1', 'Ablation mean2', 'Ablation angle2'] + \
